...
class IPTVWorker(QThread):
    ...
    def run(self):
        try:
            ...
            if self.epg_url:
                epg_db = self._parse_epg(self.epg_url, headers)

            if self.proto == "M3U":
                channels = self._process_m3u(headers)
            elif self.proto == "XTREAM":
                channels = self._process_xtream(headers)
            elif self.proto == "STALKER":
                channels = self._process_stalker(headers)
            
            self.finished.emit(channels, epg_db, f"Success: {len(channels)}")
        except Exception as e:
            self.error.emit(str(e))

    def _process_m3u(self, h):
    ...
    def _process_stalker(self, h):
        ...
        return [{
            ...
            "name": c.get('name'),
            "logo": "",
            "url": c.get('url', '').split(' ')[-1],
            "catchup": "none"
        } for c in data.get('js', [])]

    def _parse_epg(self, url, h):
