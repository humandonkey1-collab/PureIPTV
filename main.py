import sys, os, requests, re, json, gzip, sqlite3, locale, threading
from datetime import datetime, timezone
from xml.etree import ElementTree as ET
from PySide6.QtGui import QGuiApplication
from PySide6.QtQml import QQmlApplicationEngine
from PySide6.QtCore import QObject, Slot, Signal, Property, QAbstractListModel, Qt, QThread, QVariant
...
class IPTVWorker(QThread):
    ...
    def __init__(self, proto, host, epg_url="", user="", pwd="", mac=""):
        super().__init__()
        self.proto, self.host, self.epg_url = proto, host.strip().rstrip('/'), epg_url.strip()
        self.user, self.pwd, self.mac = user, pwd, mac
    ...
    def run(self):
        try:
            ...
            if self.epg_url: epg_db = self._parse_xmltv(self.epg_url, headers)

            if self.proto == "M3U":
                r = requests.get(self.host, headers=headers, timeout=15)
                pattern = re.compile(r'#EXTINF:.*?(?:tvg-id="(.*?)")?.*?(?:tvg-logo="(.*?)")?.*?(?:catchup="(.*?)")?.*?,(.*?)\n(http.*?)(?:\n|$)', re.M)
                for m in pattern.finditer(r.text):
                    channels.append({"id": m.group(1) or m.group(4).strip(), "logo": m.group(2) or "", "name": m.group(4).strip(), "url": m.group(5).strip()})
                ...
                for i in res: channels.append({"id": str(i.get('epg_channel_id', i.get('name'))), "name": i.get('name'), "logo": i.get('stream_icon', ''), "url": f"{self.host}/live/{self.user}/{self.pwd}/{i.get('stream_id')}.ts"})
            elif self.proto == "STALKER":
                h = headers.copy(); h["X-User-MAC"] = self.mac; h["Cookie"] = f"mac={self.mac}"
                ...
                for c in res['js']: channels.append({"id": str(c.get('tvg_id', c.get('name'))), "name": c['name'], "logo": "", "url": c['url'].split(' ')[-1]})
            self.finished.emit(channels, epg_db, f"Загружено: {len(channels)}")
    ...
    def _parse_xmltv(self, url, h):
        ...
        for p in tree.findall('programme'):
            ...
            epg[cid].append({"title": p.findtext('title'), "start_raw": start, "time": f"{start[8:10]}:{start[10:12]}", "desc": p.findtext('desc') or ""})
        return epg
...
class EPGModel(QAbstractListModel):
    ...
    def data(self, index, role):
        ...
        return self._items[index.row()].get({201: "title", 202: "time", 203: "desc", 204: "start_raw"}.get(role))
    def roleNames(self): return {201: b"title", 202: b"displayTime", 203: b"desc", 204: b"startRaw"}
    def set_data(self, d): self.beginResetModel(); self._items = d; self.endResetModel()

class IPTVCore(QObject):
    statusChanged = Signal(); loadFinished = Signal()
    ...
    def connect(self, proto, host, epg, user, pwd, mac):
        ...
        self.w.error.connect(lambda e: self._on_error(e)); self.w.start()
    ...
    def _on_error(self, e): self._status = f"Error: {e}"; self.statusChanged.emit()

    @Slot(str)
    def updateEPG(self, cid): self._epg_model.set_data(self._epg_data.get(cid, []))
