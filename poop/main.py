...
class StreamOptimizer:
    ...
    force_lowest_variant, disable_logos, skip_country_detect можно включить
    вручную из меню 💰 на OSD плеера, если нужна максимальная экономия.
    """
    ...
    def __init__(self):
        ...
        # Если True, в мастер-плейлисте всегда выбирается самый низкий ABR-вариант
        # даже при выбранном качестве ultra/high и т.д. Включается вручную из меню 💰.
        self.force_lowest_variant = False
        # Лимит трафика для теста пропускной способности (KiB).
        # 0 = пропустить тест целиком (он сам по себе жрёт ~512 KiB впустую на каждый канал).
        self.bandwidth_probe_kib = 0
        
    def detect_bandwidth(self, url, proxy_url=None):
        """Определяет пропускную способность канала. Не тратит трафик, если выключено."""
...
class HLSCache:
    ...
    def _get_quality_variant(self, content, bandwidth):
        ...
        # === ЖЁСТКИЙ РЕЖИМ ЭКОНОМИИ: всегда самый дешёвый вариант ===
        if stream_optimizer.force_lowest_variant:
            chosen = variants[0]  # уже отсортированы по возрастанию
            print(f"[Optimizer] 💰 TRAFFIC SAVER: forced LOWEST variant "
                  f"{chosen['resolution']} ({chosen['bw_kbps']:.0f} Kbps) — экономия трафика!")
        else:
            # Обычная логика по выбранному уровню качества
            ...
            print(f"[Optimizer] Selected variant: {chosen['resolution']} "
                  f"({chosen['bw_kbps']:.0f} Kbps) for target: "
                  f"{target_bw_kbps:.0f} Kbps (level: {quality_level})")
...
class IPTVCore(QObject):
    ...
    def __init__(self, engine):
        ...
        # Логотипы и страна нужны → НЕ трогаем. Качество выбирает пользователь.
        # Любой из этих тумблеров можно включить из меню 💰 на OSD плеера,
        # если нужна максимальная экономия (например, на мобильном интернете).
        self._disable_logos = False       # Логотипы каналов — качаем нормально
        self._skip_country_detect = False # Детект страны через ipapi.co — работает
        # Параллельно в StreamOptimizer: force_lowest_variant=False
        # (4K остаётся 4K, mpv сам выбирает максимальный вариант по умолчанию).
        ...
        if HAS_MPV:
            try:
                self.player = mpv.MPV(
                    ...
                    # Это НЕ урезает качество — это просто потолок, выше которого
                    # плеер не буферизует. На паузе/перемотке назад не держим лишнего.
                    keep_open='yes',
                    keep_open_pause='no',
                    hr_seek='yes',
                    network_timeout='20',
                    ...
                    demuxer_max_bytes='200MiB',     # максимум 200 МБ (ТЗ)
                    demuxer_max_back_bytes='10MiB', # назад только 10 МБ (перемотка)
                    demuxer_readahead_secs='60',    # вперёд максимум 60 с
                    demuxer_hysteresis_secs='10',   # НЕ качать дальше, пока в кэше есть >=10 с
                    ...
                    # ДЛЯ LIVE СТРИМОВ
                    force_seekable='yes',
                    # hls-bitrate НЕ выставлен: по умолчанию mpv выберет максимальный
                    # вариант, как и просит пользователь ("4K остаётся 4K").
                    # audio_channels / sid / alang НЕ выставлены: пользователь сам
                    # выбирает субтитры и формат аудио через меню плеера.
                )
                
                print("✅ MPV initialized (cache capped 60s/200MiB, hysteresis=10s; "
                      "quality, subs, audio — управляются пользователем)")
    ...
    # =========================================================
    # СВОЙСТВА ЖЁСТКОЙ ЭКОНОМИИ ТРАФИКА (QML может их переключать)
    # =========================================================
    bandwidthSaverChanged = Signal()

    @Property(bool, notify=bandwidthSaverChanged)
    def disableLogos(self):
        """Не качать логотипы каналов в QML Image (экономит ~50–500 КБ на канал)"""
        return self._disable_logos
    ...
    @Property(bool, notify=bandwidthSaverChanged)
    def skipCountryDetect(self):
        """Не делать HTTP-запрос к ipapi.co на каждый канал (экономит ~1–5 КБ на канал)"""
        return self._skip_country_detect
    ...
    @Property(bool, notify=bandwidthSaverChanged)
    def forceLowestVariant(self):
        """В HLS-плейлисте всегда брать самый дешёвый вариант (главный рычаг экономии!)"""
        return stream_optimizer.force_lowest_variant
    ...
    def forceLowestVariant(self, val):
        ...
        if stream_optimizer.force_lowest_variant != new:
            ...
            # Сбросить HLS-кэш, чтобы новый выбор варианта применился немедленно
            HLSCache._global_cache.clear()
            self.bandwidthSaverChanged.emit()
            print(f"[SAVER] Принудительный lowest ABR variant: "
                  f"{'ВКЛ' if new else 'ВЫКЛ'}")
    ...
    def _apply_stream_optimizations(self):
        """Применяет инфраструктурные оптимизации (только потолки кэша, без урезания качества)."""
        if not HAS_MPV or not self.player:
            return
            
        try:
            # === СТРОГИЕ ЛИМИТЫ КЭША (ТЗ: 200 МБ / 60 секунд) ===
            # Не урезаем качество — просто не даём плееру раздувать буфер сверх бюджета.
            try: self.player['cache'] = 'yes'
            except: pass
            try: self.player['cache-secs'] = '60'                  # 60 секунд видео
            except: pass
            try: self.player['demuxer-max-bytes'] = '200MiB'        # 200 МБ (ТЗ)
            except: pass
            try: self.player['demuxer-max-back-bytes'] = '10MiB'    # 10 МБ назад
            except: pass
            try: self.player['demuxer-readahead-secs'] = '60'       # 60 с readahead
            except: pass
            try: self.player['demuxer-hysteresis-secs'] = '10'      # стоп докачки при >=10 с в кэше
            except: pass
            # hls-bitrate НЕ выставляем — пользователь сам решает.
            # audio-channels / sid / alang НЕ выставляем — пользователь сам решает.

            # === LIVE / KEEP-ALIVE ===
            try: self.player['live-keepalive'] = 'yes'
            except: pass
            ...
            print("[Optimizer] ✅ Cache capped (60s/200MiB, hysteresis=10s); "
                  "quality/subs/audio — выбор пользователя")
        except Exception as e:
            print(f"[Optimizer] Optimization error: {e}")

    def _detect_country(self, url, cat, name):
        code, cn = detect_country(cat, name)
        # === РЕЖИМ ЭКОНОМИИ ТРАФИКА: пропускаем сетевой DNS и HTTP к ipapi.co ===
        # (DNS + запрос к ipapi.co — это 1–5 КБ впустую на каждый канал)
        if code == "ALL" and not self._skip_country_detect:
            ...
            print(f"[SAVER] Пропускаем DNS+HTTP для определения страны канала")
