...
def get_local_ip():
    """Определяет локальный IP-адрес в LAN для подключения внешних плееров (Televizo, TiviMate, VLC)."""
    try:
...
class HLSProxyHandler(BaseHTTPRequestHandler):
    """Оптимизированный HLS Proxy с буферизацией, переподключением и поддержкой Televizo."""
    ...
    def do_GET(self):
        try:
            if self.path.startswith('/playlist'):
                ...
                return
            elif self.path.startswith('/livepipe/'):
    ...
    def _handle_playlist(self):
        ...
        try:
            ...
            if 'id=' in self.path:
                ...
                try:
                    m = re.search(r'/playlist/(\d+)', self.path)
                    if m:
                        pid = int(m.group(1))
                except Exception:
                    pid = None
            ...
            if pid and pid > 0:
                rows = db.fetchall("SELECT channels, movies, series FROM playlists WHERE id=?", (pid,))
            else:
                if HLSProxyHandler.core_ref and HLSProxyHandler.core_ref.current_playlist_id:
                    rows = db.fetchall("SELECT channels, movies, series FROM playlists WHERE id=?", (HLSProxyHandler.core_ref.current_playlist_id,))
                else:
    ...
    def _handle_m3u8(self, url):
        ...
        # ВСЕГДА проксируем вложенные сегменты и подплейлисты (.ts, .aac, .mp4, .m3u8),
        # чтобы Televizo и внешние плееры не получали ошибки 403/401/404/500 от сервера источника!
        proxy_sub = True
            ...
            if line.startswith('#'):
                if 'URI=' in line:
                    def replace_uri(match):
                        ...
                        proxied = f"{proxy_base}/hls/{urllib.parse.quote(uri_val, safe='')}"
                        return f'URI="{proxied}"'
                    line = re.sub(r'URI="([^"]*)"', replace_uri, line)
                lines.append(line)
            else:
                absolute = make_absolute(line, base_url, url)
                proxied_seg = f"{proxy_base}/hls/{urllib.parse.quote(absolute, safe='')}"
    ...
    def _handle_generic(self, url):
        ...
        try:
            ...
            print(f"⚠️ [HLS] Generic stream error: {e}")
            try:
                self.send_error(500)
            except Exception:
                pass
...
def start_hls_proxy(proxy_url=None, port=8899, core=None):
    ...
    try:
        ...
        HLSProxyHandler.core_ref = core
        # Привязываемся к 0.0.0.0, чтобы Televizo на Android TV Box или телефоне в LAN мог подключиться!
        _hls_server = HLSProxyServer(('0.0.0.0', port), HLSProxyHandler)
        threading.Thread(target=_hls_server.serve_forever, daemon=True).start()
        print(f"✅ [HLS Proxy] Started on 0.0.0.0:{port} (Accessible via http://{local_ip}:{port})")
        return f"http://{local_ip}:{port}"
    except Exception as e:
...
def get_proxied_url(url, port=8899):
    if not url:
        return url
    local_ip = get_local_ip()
    return f"http://{local_ip}:{port}/hls/{urllib.parse.quote(url, safe='')}"
...
class IPTVCore(QObject):
    ...
    def _needs_proxy(self, url):
        ul = url.lower()
        # Всегда используем локальный прокси для стабильности потоков (особенно капризных ссылок)
        return True

    def _play_url(self, url):
        """Все потоки пускаем через наш HLS прокси для обхода защит и стабильности"""
        if self._needs_proxy(url):
            start_hls_proxy(None, core=self)
            local_ip = get_local_ip()
            if self._is_live_stream(url, self._last_start_raw):
                proxied = f"http://{local_ip}:8899/livepipe/{urllib.parse.quote(url, safe='')}"
    ...
    def copyToClipboard(self, text):
        try:
            ...
            print(f"📋 Copied to clipboard for Televizo: {text[:60]}...")
            return True
        except Exception as e:
