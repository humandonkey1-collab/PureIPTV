                            ...
                            RowLayout {
                                ...
                                Rectangle {
                                    ...
                                    Image {
                                        id: chanLogo
                                        anchors.fill: parent
                                        // === РЕЖИМ ЭКОНОМИИ ТРАФИКА: пропускаем загрузку логотипа ===
                                    ...
                                    Label {
                                        ...
                                        anchors.centerIn: parent
                                        // Показываем ТВ-заглушку, если логотип отключен экономией или не загрузился
            ...
            ColumnLayout {
                ...
                Rectangle {
                    ...
                    Image {
                        id: playerLogo
                        anchors.fill: parent
                        // === РЕЖИМ ЭКОНОМИИ ТРАФИКА: пропускаем логотип и на экране плеера ===
                    ...
                    Label {
                        ...
                        font.pixelSize: 55 * window.scaleFactor
                        anchors.centerIn: parent
                        visible: backend.disableLogos || !window.selCh || !window.selCh.logo || playerLogo.status !== Image.Ready
                    }
                }
        ...
        Rectangle {
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    // Кнопка экономии трафика (открывает меню настройки режима saver)
                    Button {
                        id: saverBtn
                        text: "💰"
                        font.pixelSize: window.fontSizeBody
                        flat: true
                        highlighted: backend.forceLowestVariant
                        ToolTip.visible: hovered
                        ToolTip.text: backend.forceLowestVariant
                            ? "Экономия трафика ВКЛ (берём самый низкий ABR)"
                            : "Экономия трафика ВЫКЛ"
                        onClicked: saverMenu.open()
                    }
                }

                // Меню настройки режима экономии трафика
                Menu {
                    id: saverMenu
                    title: "💰 Экономия трафика"

                    MenuItem {
                        text: (backend.forceLowestVariant ? "✅ " : "⬜ ") + "Самый низкий битрейт в HLS"
                        onClicked: backend.forceLowestVariant = !backend.forceLowestVariant
                    }
                    MenuItem {
                        text: (backend.disableLogos ? "✅ " : "⬜ ") + "Не качать логотипы каналов"
                        onClicked: backend.disableLogos = !backend.disableLogos
                    }
                    MenuItem {
                        text: (backend.skipCountryDetect ? "✅ " : "⬜ ") + "Не определять страну канала"
                        onClicked: backend.skipCountryDetect = !backend.skipCountryDetect
                    }
                    MenuSeparator {}
                    MenuItem {
                        text: "ℹ️ Кэш: 60 с / 200 МБ, без субтитров, стерео"
                        enabled: false
                    }
                }
